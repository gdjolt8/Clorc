...
ClorASTExpr* init_ast_expr(int type) {
  ClorASTExpr* ast = (ClorASTExpr*)calloc(1, sizeof(ClorASTExpr));
  ast->ast_type = type;
...
void free_ast_expr(ClorASTExpr* ast) {
  ...
  if (ast->ast_type == AST_VARIABLE_DEFINITION) {
    free_clor_data_type(ast->data_type);
...
char const* ast_type_to_string(int type) {
  switch (type)
  {
    ...
    case AST_FUNCTION_CALL: return "[Function]";
    case AST_FUNCTION_DEFINITION: return "Function definition";
